...
stcar_edge <-mutate(stcar_edge, addr_to_geocode = (paste(ADDRNO, ADDRST, ADDRSF, "cincinnati, OH")))
...
# Change this to PKWY
stcar_edge$ADDRSF[c(which(stcar_edge$ADDRSF == "PW"))] <- "PKWY"
...
for (ii in seq(startindex, nrow(stcar_edge))){
  print(paste("Working on index", ii, "of", nrow(stcar_edge)))
  #query the google geocoder - this will pause here if we are over the limit.
  result = getGeoDetails(stcar_edge$addr_to_geocode[ii]) 
  print(result$status)     
  result$index <- ii
...
stcar_edge<-bind_cols(stcar_edge, geocoded)
...
CinciDensityMap <-(myMap)

# provide  data to aestheic mappings
CinciMap <- CinciMap + geom_point(aes( x = as.numeric(long), y = as.numeric(lat), alpha = 1.0, col = EXLUCODE, size= as.numeric(ACREDEED *10)), data= stcar_edge)

print(CinciMap)

# Added a layer for density 
CinciDensityMap <- CinciMap + stat_density2d(
