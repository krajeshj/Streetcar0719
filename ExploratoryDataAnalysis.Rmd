...
# n <- n + theme(axis.text.x = element_text(angle=305))
 ...
 #m <- m + scale_fill_manual(labels=exlu_desc)
# 
# Theme
 m <- m + coord_flip()
...
m <-(ggplot)
 ...
 # Data + aes + geom + facets + stats + Co-ords
 m <- m + xlab("Existing Land Use Code ")
 #m <- m + scale_fill_manual(labels=c("?","C - Commercial", "ED - ", "HI - ", "IN - ", "LI -" ,"MF - Multi-Family", "MU - Multi unit", "O - Office space", "PR - ", "PS - ", "PU - ", "SF - Single Family", "TF - Three Family Dwelling ", "VA - "))
# 
  # Theme
 m <- m + coord_flip()
